...
import org.example.bigdata.realtime.flink.streaming.funs.orders.OrdersAggFun.{OrderDetailTimeAggFun, OrderTopNKeyedProcessFun, OrderTrafficTimeWindowFun}
import org.example.bigdata.realtime.flink.streaming.rdo.QRealtimeDO.{OrderDetailData, OrderTrafficDimData, OrderTrafficDimMeaData}
...
import org.example.bigdata.realtime.flink.streaming.schema.OrderStatisKSchema
/**
 * 旅游订单业务数据的实时统计排名
...
object OrdersTopnStatisHandler {
  ...
  def handleOrdersWindowTopNJob(appName:String, groupID:String, fromTopic:String, toTopic:String, topN:Long):Unit = {
    try{
       ...
       */
      val orderDetailDStream:scala.DataStream[OrderDetailData] = FlinkHelper.createOrderDetailDStream(env, groupID, fromTopic,timeChar)

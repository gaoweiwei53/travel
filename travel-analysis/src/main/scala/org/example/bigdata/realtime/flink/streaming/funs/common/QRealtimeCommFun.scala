package org.example.bigdata.realtime.flink.streaming.funs.common
...
import org.example.bigdata.realtime.flink.streaming.rdo.QRealtimeDO.{OrderDetailData, OrderMWideData, OrderWideData}
import org.example.bigdata.realtime.utils.{CommonUtil, DBDruid, JsonUtil, PropertyUtil, RedisCache}
    ...
    override def asyncInvoke(input: OrderDetailData, resultFuture: ResultFuture[OrderWideData]): Unit = {
      try {
        ...
        if(useLocalCache){
          ...
          val key = dbQuery.table
          val dataJson : java.util.Map[String,String] = redisCmd.hgetall(key)
          productInfo =  dataJson.getOrDefault(orderProductID,"")
        }
        val productRow : java.util.Map[String,Object] = JsonUtil.json2object(productInfo, classOf[java.util.Map[String,Object]])
    ...
    override def asyncInvoke(input: OrderDetailData, resultFuture: ResultFuture[OrderMWideData]): Unit = {
      try {
        ...
        if(StringUtils.isNotEmpty(productJson)){
          val productRow : java.util.Map[String,Object] = JsonUtil.json2object(productJson, classOf[java.util.Map[String,Object]])
        ...
        if(StringUtils.isNotEmpty(pubJson)){
          val pubRow : java.util.Map[String,Object] = JsonUtil.json2object(pubJson, classOf[java.util.Map[String,Object]])

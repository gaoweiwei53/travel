package org.example.bigdata.realtime.flink.utils
...
object DateUtil {
   ...
   */
    ...
    def dateFormat(time: Long, pattern: String): String = {
      ...
      val datetime = LocalDateTime.ofInstant(new Date(time).toInstant,
    ...
    def dateFormat(date: Date): String = {
      ...
      val datetime = LocalDateTime.ofInstant(date.toInstant,
        ZoneId.systemDefault())
